MODULE SAO_OMIL2_median_module
...
CONTAINS
  ...
  REAL (KIND=r4) FUNCTION median_r4 (n, x)
    ...
    ! Local variables
    ! ---------------
    REAL    (KIND=r4), DIMENSION (n) :: tmp

    tmp(1:n) = x(1:n)               ! make a copy

    CALL sort_r4(n, tmp)              ! sort the copy
    IF (MOD(n,2) == 0) THEN           ! compute the median
       median_r4 = (tmp(n/2) + tmp(n/2+1)) / 2.0_r4
    ELSE
       median_r4 = tmp(n/2+1)
    END IF
  ...
  INTEGER (KIND=i4) FUNCTION FindMinimumPosition_r4(n, x, start, end) RESULT (minpos)
    ...
    ! ---------------
    INTEGER (KIND=i4),                 INTENT (IN) :: n
    REAL    (KIND=r4), DIMENSION(1:n), INTENT (IN) :: x
    INTEGER (KIND=i4),                 INTENT (IN) :: start, end
  ...
  END FUNCTION  FindMinimumPosition_r4
  ...
  SUBROUTINE  sort_r4(n, x)
    ...
    ! Modified variable
    ! -----------------
    REAL (KIND=r4), DIMENSION(n), INTENT (INOUT) :: x
    ...
    DO i = 1, n-1                                      ! except for the last
       minpos = FindMinimumPosition_r4 ( n, x, i, n )  ! find min from this to last
       CALL swap_r4 (x(i), x(minpos))                   ! swap this and the minimum
    END DO

    RETURN
  END SUBROUTINE  sort_r4

  REAL (KIND=r8) FUNCTION mean (n, x)
...
END MODULE  SAO_OMIL2_median_module
